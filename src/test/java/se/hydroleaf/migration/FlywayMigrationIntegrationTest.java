...
class FlywayMigrationIntegrationTest {
    ...
    static void configureDataSource(DynamicPropertyRegistry registry) {
        ...
        registry.add("mqtt.publishEnabled", () -> "false");
    ...
    void migrationBackfillsNotificationFlagsForExistingUsers() throws Exception {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            statement.executeUpdate(
                    "insert into app_user (email, password, role, active, status, invited, created_at) "
                            + "values ('test@example.com', 'secret', 'ADMIN', true, 'ACTIVE', false, now())"
            );
        }
        ...
        try (Connection connection = dataSource.getConnection();
             ...
             ResultSet resultSet = statement.executeQuery(
                     "select order_confirmation_emails, pickup_ready_notification from app_user where email = 'test@example.com'"
             )) {
