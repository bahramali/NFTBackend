...
- **عدم دسترسی عمومی به فهرست کاربران**: دسترسی به `GET /api/users` و `GET /api/users/{id}` فقط برای `SUPER_ADMIN` یا ادمین دارای مجوز `TEAM` مجاز است؛ سایر نقش‌ها پاسخ `403 Forbidden` می‌گیرند. هدر احراز هویت نامعتبر یا غایب باعث `401 Unauthorized` می‌شود.
...
- `displayName` (اختیاری): نام نمایشی حداکثر ۱۲۸ کاراکتری.
- `permissions` (الزامی): آرایهٔ مقادیر enum شامل یکی از [`ADMIN_DASHBOARD`, `REPORTS`, `TEAM`]. مقدار نامعتبر خطای ۴۰۰ با پیام واضح برمی‌گرداند.
- `expiresInHours` (اختیاری): عدد صحیح مثبت برای تنظیم اعتبار دعوت (پیش‌فرض ۴۸ ساعت).
- `expiresAt` (اختیاری): تاریخ/زمان ISO-8601 در آینده برای منقضی‌شدن دعوت؛ در صورت ارسال، بر `expiresInHours` اولویت دارد.

> ارسال فیلدهای ناشناخته در بدنه باعث خطای ۴۰۰ می‌شود و پاسخ شامل لیست خطا با نام فیلد و پیام خوانا خواهد بود.
